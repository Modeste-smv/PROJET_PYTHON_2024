...
def process_expirations(symbols, expiration_dates=None):
    ...
    for sym in symbols:  # Utiliser 'sym' au lieu de 'symbol' pour éviter toute confusion
        ...
        if not expiration_dates:
            ...
            expirations_to_process = [date for date in expiration_dates if date in available_expirations]
        ...
        for expiration_date in expirations_to_process:
            try:
                ...
                print(f"Erreur lors de la récupération des données pour {sym} à la date {expiration_date}: {e}")
